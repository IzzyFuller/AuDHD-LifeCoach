...
class ProcessCommunication:
    ...
    def execute(
        self, communication_dto: CommunicationRequestDTO
    ) -> CommunicationResponseDTO:
        """
        ...
        # Create response DTO
        response = CommunicationResponseDTO(processed=True, reminders=reminder_dtos)
    ...
    def _format_reminder_for_publishing(
        self, reminder_dto: ReminderResponseDTO
    ) -> Dict[str, Any]:
        """
        ...
        return {
            ...
            "commitment": {
                ...
                "start_time": (
                    reminder_dto.commitment_start_time.isoformat()
                    if reminder_dto.commitment_start_time
                    else None
                ),
                "end_time": (
                    reminder_dto.commitment_end_time.isoformat()
                    if reminder_dto.commitment_end_time
                    else None
                ),
            },
        }
    ...
    def _publish_results(
        ...
        message = {
            ...
            "original_communication": {
                ...
                "recipient": communication_dto.recipient,
                "timestamp": (
                    communication_dto.timestamp.isoformat()
                    if communication_dto.timestamp
                    else None
                ),
            },
            "processed": response_dto.processed,
            "reminders": [
                self._format_reminder_for_publishing(r) for r in response_dto.reminders
            ],
        }
        ...
        if success:
            logger.info(
                f"Published communication results with {len(message['reminders'])} reminders"
            )
        else:
