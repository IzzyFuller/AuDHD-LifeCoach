...
class RabbitMQSettings:
    ...
    publish_exchange_name: str = os.getenv('RABBITMQ_PUBLISH_EXCHANGE', 'audhd_lifecoach')
    ...
    def __post_init__(self):
        ...
        if self.connection_attempts <= 0:
            raise ValueError(f"Connection attempts must be positive: {self.connection_attempts}")
