...
class TestMessageConsumerFlow:
    ...
    def test_basic_message_flow_with_one_commitment(
        self,
        mock_pika_connection,
        rabbitmq_adapter,
        message_consumer_service,
        setup_message_delivery,
        ...
        # Start the consumer service - this will connect to RabbitMQ and start consuming
        message_consumer_service.start()
        ...
        mock_channel.basic_ack.assert_called_once_with(
            delivery_tag=message_data["message_id"]
        )
    ...
    @pytest.mark.integration
    def test_error_handling_for_invalid_message(
        self,
        mock_pika_connection,
        rabbitmq_adapter,
        message_consumer_service,
        ...
        mock_channel.basic_reject.assert_called_once_with(
            delivery_tag=invalid_message_data["message_id"], requeue=False
        )
    ...
    def test_multiple_messages_processing(
        ...
        assert mock_channel.basic_ack.call_count == len(
            messages
        ), f"Expected basic_ack to be called {len(messages)} times"
    ...
    @pytest.mark.integration
    def test_json_decode_error_in_flow(
        self,
        mock_pika_connection,
        rabbitmq_adapter,
        message_consumer_service,
    ...
    @pytest.mark.integration
    def test_exception_during_processing_in_flow(
        self,
        mock_pika_connection,
        rabbitmq_adapter,
        message_consumer_service,
        ...
        with patch.object(
            ...
            mock_channel.basic_reject.assert_called_once_with(
                delivery_tag=message_data["message_id"], requeue=True
            )
    ...
    @pytest.mark.integration
    def test_disconnect_cleans_up_resources(
        self, mock_pika_connection, rabbitmq_adapter
    ):
        """
